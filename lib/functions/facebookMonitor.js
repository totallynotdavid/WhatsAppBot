...
const remoteNumber = process.env.ADMIN_NUMBER;
...
if (fs.existsSync(lastPostIdFile)) {
    ...
    console.log(
        `No last post ID found, it should be created after the first check`
    );
}
...
async function checkForNewPosts(client) {
    const latestPost = await getLatestPost(FACEBOOK_PAGE_ID);
