...
async function addGPTConversations(phoneNumber, message, group, tableName, sender='user') {
  try {
    if (message === null) {
...
async function fetchLastNMessages(phoneNumber, group, n) {
  const { data, error } = await supabase
    ...
    .eq('conversation_id', `${group}_${phoneNumber}`)
  ...
  return data.map(d => ({role: d.sender === 'assistant' ? 'assistant' : 'user', content: d.message})).reverse();
}
