...
async function addGPTConversations(
    ...
    try {
        if (message === null) {
            console.log(`El mensaje es null. Logs: [sender: ${sender}, phoneNumber: ${phoneNumber}, group: ${group}]`,);
            return;
        }
...
async function searchPremiumGroup(group_id) {
    return searchTable(
        `premium_groups`, `group_id`, group_id
    );
}

async function addPremiumGroup(
    group_id, group_name, phoneNumber
) {
...
async function updatePremiumGroup(
    group_id, group_name, phoneNumber
) {
    const { data, error } = await supabase
        .from(`premium_groups`)
...
async function fetchLastNMessages(
    phoneNumber, group, n
) {
    try {
        const { data, error } = await supabase
