...
const PDF_PATH = path.join(process.cwd(), 'pdf');
...
class FolderDatabase {
  ...
  async load() {
		...
		const newFileCount = [...newIds].filter(id => !existingIds.has(id)).length;
	...
	async refresh() {
		try {
			...
			console.log(`La base de datos ha sido actualizada. Se encontraron ${result.totalFiles} archivos en total, ${result.newFiles} son nuevos.`);
			return result;
		} catch (error) {
  ...
  scheduleRefresh() {
    ...
    setTimeout(() => {
      console.log('Updating the SQLite database...');
      this.load()
        .then(() => console.log('The SQLite database has been updated successfully.'))
        .catch((error) => console.error('An error occurred while updating the SQLite database:', error))
...
async function downloadFilesFromGoogleDrive(query) {
  ...
  try {
    ...
    await fsPromises.mkdir(PDF_PATH, { recursive: true });
    const filePath = path.join(__dirname, PDF_PATH, `${realFileId}.pdf`);
