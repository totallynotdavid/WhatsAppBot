...
const TOKEN_PATH = path.join(
    process.cwd(), `data`, `driveToken.json`
);
...
class FolderDatabase {
        ...
        await new Promise((resolve, reject) => {
            this.db.serialize(() => {
                ...
                files.forEach((file) => {
                    ...
                    stmt.run(
                        file.id, file.name, parentsStr, file.webViewLink, (err) => {
                            if (err) {
                                reject(err);
                            }
                        }
    ...
    async refresh() {
        try {
            ...
            console.error(`An error occurred while updating the SQLite database:`,
                error,);
            throw error;
        }
    }
